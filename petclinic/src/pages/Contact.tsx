  ...
  return (
    <>
      <section className="bg-(--color-teal-50) py-14 text-center">
        ...
        <p className="mx-auto mt-4 max-w-2xl px-4 text-sm text-(--color-ink-soft)">We'd love to hear from you — reach out by phone, WhatsApp, or the form below.</p>
      </section>
      <section className="mx-auto grid max-w-7xl gap-8 px-4 py-16 lg:grid-cols-2 lg:px-8">
        <div className="space-y-4">
          {[
            ...
            { icon: Clock3, title: 'Hours', body: `${clinicSettings.hours_weekday}\n${clinicSettings.hours_sunday}` },
          ].map(({ icon: Icon, title, body }) => (
            <div key={title} className="flex items-start gap-4 rounded-2xl border border-(--color-teal-100) bg-white p-5 shadow-[var(--shadow-card)]">
              <span className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-(--color-teal-50) text-(--color-teal-600)"><Icon size={20} /></span>
              <div>
        ...
        <div className="rounded-3xl bg-white p-6 shadow-[var(--shadow-card)] sm:p-8">
          ...
          <div className="mt-5"><InquiryForm /></div>
        </div>
      </section>
      <MapSection />
    </>
