    ...
    <>
      <section className="bg-(--color-teal-50) py-14 text-center">
        ...
        <p className="mx-auto mt-4 max-w-2xl px-4 text-sm text-(--color-ink-soft)">
          Practical, vet-approved advice for keeping your pets healthy and happy.
        </p>
      </section>
        ...
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {blogPosts.map((post) => (
            <Link
              key={post.id}
              to={`/blog/${post.slug}`}
              className="group overflow-hidden rounded-2xl bg-white shadow-[var(--shadow-card)]"
            >
              <img
              ...
              <div className="p-5">
                ...
                <h2 className="font-display mt-1 text-base font-semibold text-(--color-ink) group-hover:text-(--color-teal-600)">
                  {post.title}
                </h2>
