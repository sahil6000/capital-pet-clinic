      ...
      <div className="mx-auto max-w-5xl px-4 lg:px-8">
        <div className="text-center">
          ...
          <p className="mx-auto mt-4 max-w-xl text-sm text-(--color-ink-soft)">
            Tell us about your pet and pick a time that works for you. We confirm every booking on
            WhatsApp within minutes during clinic hours.
          </p>
        </div>
        ...
        <div className="mt-10 grid gap-6 sm:grid-cols-3">
          {[
            ...
            <div key={label} className="flex items-center gap-3 rounded-2xl bg-white p-4 shadow-[var(--shadow-card)]">
              <span className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-(--color-teal-50) text-(--color-teal-600)">
                <Icon size={18} />
              </span>
        ...
        <div className="mt-8 rounded-3xl bg-white p-6 shadow-[var(--shadow-soft)] sm:p-10">
          ...
          {clinicSettings.payments_enabled ? (
            <p className="mt-5 text-center text-xs text-(--color-ink-soft)">
              Online consultation fee payment is available after your appointment is confirmed.
              You&apos;ll receive a secure payment link on WhatsApp or email.
            </p>
          ) : null}
        </div>
      </div>
