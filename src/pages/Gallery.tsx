    ...
    <>
      <section className="bg-(--color-teal-50) py-14 text-center">
        ...
        <p className="mx-auto mt-4 max-w-2xl px-4 text-sm text-(--color-ink-soft)">
          A glimpse into our clinic, our team, and the happy pets we care for.
        </p>
      </section>
          ...
          {categories.map((category) => (
            <button
              ...
              className={`rounded-full px-4 py-2 text-sm font-medium transition-colors focus-ring ${
                active === category.key
                  ? 'bg-(--color-teal-600) text-white'
                  : 'bg-(--color-teal-50) text-(--color-ink-soft) hover:bg-(--color-teal-100)'
              }`}
            >
              {category.label}
            </button>
